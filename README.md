...
4.  **Stand Up Script**: Click the scroll icon (📜), then the gear icon (⚙️) inside the modal.
    ...
    - Configure column mappings for each selected board by clicking its gear icon (⚙️). Ensure you map your JIRA statuses correctly to "In Progress", "In QA", "Done", and "Cancelled". Verify/enter the Story Points Field ID.
...
- **Board & Project Configuration**:
  ...
  - Configure status mappings (In Progress, In QA, Done, Cancelled) for each selected board to standardize statuses sent to the external API.
...
- **Column Configuration Modal**:
  ...
  - Shows available columns and allows assigning them to categories (In Progress, In QA, Done, Cancelled) via drag-and-drop or selection modals.
